...
# np.random.seed(1234)
'''Reward scaling from [55, 160] to [0, 1]'''
ac_reward_floor = 55
ac_reward_ceil = 160
reward_floor = ac_reward_floor / 100
reward_range = ac_reward_ceil - ac_reward_floor
...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, n_RB, n_neighbor, n_pkg=2, lambda_V2I=0.1):
        ...
        self.bandwidth = int(1.5e6)  # bandwidth per RB, 1.5 MHz
        # self.bandwidth = 1500
        # self.demand_size = int((4 * 190 + 300) * 8 * 2)  # V2V payload: 1060*2 Bytes every 100 ms
        self.demand_size = int((4 * 190 + 300) * 8 * self.n_pkg)  # V2V payload: 1060 Bytes every 100 ms
        # self.demand_size = 20

        self.lambda_V2I = lambda_V2I
    ...
    def renew_neighbor(self):
        ...
        for i in range(len(self.vehicles)):
            sort_idx = np.argsort(Distance[:, i])  # Output the distance index for each vehicle from small to big
    ...
    def Compute_Performance_Reward_Train(self, actions_power):
        ...
        V2I_Signals = np.zeros(self.n_RB)
        V2I_Interference = np.zeros(self.n_RB)  # V2I interference
        for i in range(len(self.vehicles)):
            for j in range(self.n_neighbor):
                # V2I_Signals[actions[i][j]] += 10 ** ((self.V2I_power_dB - self.V2I_channels_with_fastfading[i, actions[i, j]] + self.vehAntGain + self.bsAntGain - self.bsNoiseFigure) / 10)
                ...
                # 上面V2I_Interference[actions[i][j]]的计算可能有问题。因为这里重复计算了V2I_Interference了，叠加了3个信道的。
                # 但好像又没有问题，算上了所有V2V link的干扰，貌似是正确的。
        ...
        # for i in range(self.n_RB):            
            # V2I_Rate[i] = np.sum(np.log2(1 + np.divide(10**((self.V2I_power_dB + self.vehAntGain + self.bsAntGain \
            # - self.bsNoiseFigure-self.V2I_channels_abs[0:min(self.n_RB,self.n_Veh)])/10), V2I_Interference_temp[0:min(self.n_RB,self.n_Veh)])))
        
        # V2I_Signals = 10**((self.V2I_power_dB + self.vehAntGain + self.bsAntGain \
        #     - self.bsNoiseFigure-self.V2I_channels_abs[0:min(self.n_RB,self.n_Veh)])/10)  Hao Ye
        V2I_Signals = 10 ** ((self.V2I_power_dB - self.V2I_channels_with_fastfading.diagonal() + self.vehAntGain + self.bsAntGain - self.bsNoiseFigure) / 10)  # 参考Liang Le  师兄们可以帮我看看这一行有写错吗？
        ...
        for i in range(self.n_RB):  # scanning all bands
            ...
            for j in range(len(indexes)):
                ...
                V2V_Interference[indexes[j, 0], indexes[j, 1]] += 10 ** ((self.V2I_power_dB - self.V2V_channels_with_fastfading[indexes[j][0], receiver_j, i] + 2 * self.vehAntGain - self.vehNoiseFigure) / 10)
        ...
        reward_elements = V2V_Rate/1.2
        reward_elements[self.demand <= 0] = 1
        if np.min(reward_elements) == 1:
            # reward_elements = reward_elements*1.5  # 整体都传完了，就给1.5的奖励
            reward_elements = reward_elements*2  # 整体都传完了，就给2的奖励，鼓励他传完
        ...
        for i in range(self.n_RB):
            for k in range(len(self.vehicles)):
                for m in range(len(channel_selection[k, :])):
                    V2V_Interference[k, m, i] += 10 ** ((self.V2I_power_dB - self.V2V_channels_with_fastfading[k][self.vehicles[k].destinations[m]][i] + 2 * self.vehAntGain - self.vehNoiseFigure) / 10)  # 师兄的
                    # V2V_Interference[k, m, i] += 10 ** ((self.V2I_power_dB - self.V2V_channels_with_fastfading[i][self.vehicles[k].destinations[m]][i] + 2 * self.vehAntGain - self.vehNoiseFigure) / 10)  # Liang Le的
            ...
            for j in range(len(channel_selection[i, :])):
                for k in range(len(self.vehicles)):
                    for m in range(len(channel_selection[k, :])):
                        ...
                        # # global observation，Liang Le的


                        V2V_Interference[k, m, channel_selection[i, j]] += 10 ** ((self.V2V_power_dB_List[power_selection[i, j]]
                                                                                   - self.V2V_channels_with_fastfading[k][self.vehicles[k].destinations[m]][channel_selection[i,j]] + 2 * self.vehAntGain - self.vehNoiseFigure) / 10)
                        # global observation  师兄原来的
    ...
    def act_for_training(self, actions):
        ...
        # self.lambda_V2I = 0.  # 只优化V2V
        # self.lambda_V2I = 0.1
        reward = self.lambda_V2I * np.sum(V2I_Rate) / (self.n_RB * 5)\
                 + (1 - self.lambda_V2I) * np.sum(reward_elements) / (self.n_Veh * self.n_neighbor)  # 将V2I的比例设置成原来2倍
        

        # # reward scaling
        # reward = (reward - reward_floor) / reward_range
        
        return reward, V2I_Rate, V2V_Rate
